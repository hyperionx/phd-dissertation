...
The most well-known model for explaining current transient is the Space Charge Limited Current (SCLC) hypothesis, which was created for describing a general space charge within a perfect insulator \cite{many1962theory}. A variety of theories have been offered up to describe this phenomenon \cite{lampert1970current}. Since charged oxygen vacancies are thought to constitute the space charge in question, this initial model was first associated with them \cite{zafar1998oxygen}. \\

\noindent Along with the electronic current, an ionic current flows due to the drift of charged species, typically oxygen vacancies, under an applied bias. The metal electrode acts as a blocking contact for these ions, preventing them from exiting the oxide and causing them to accumulate at the interface. The resulting space charge creates an internal electric field that opposes the applied bias. This Coulombic repulsion impedes further ion migration, causing the ionic current to decay over time. Consequently, early research attributed transient behaviours to this space charge accumulation within a trap-free insulator, forming the basis of the SCLC model. \\

\noindent The original SCLC model simplifies the problem to one dimension by assuming planar conduction, where current flows uniformly between two parallel electrodes. Although the model can describe either electronic or ionic charge carriers, subsequent studies attribute the transient behavior to the migration of charged defects within the oxide \cite{zafar1998oxygen}. The mobility term, $\mu$, is therefore thought to represent moving oxygen vacancies \cite{zafar1998oxygen} or other mobile ions \cite{chen2010observation}. In this framework, the transient is driven by the ionic current component, which modulates the much larger electronic current. Originally, the current of the model is defined as:
\begin{align}
...
\noindent Here, $j(t)$ is the total current density, comprising the conduction current density, $j_{cond}$, and the displacement current density, $j_{disp}$. The displacement current accounts for the time-varying electric field, representing the rate of change of the electric displacement field. $q$ is the elementary charge in Coulombs. The term $n$ denotes the space charge concentration, referring to the net electric charge distributed throughout the oxide volume rather than at the interfaces. $\mu$ is the mobility of this space charge. $E$ is the local electric field, $L$ is the oxide thickness, $\epsilon$ is the dielectric permittivity, and $x$ is the position within the oxide ($0 \le x \le L$). Finally, Equation (\ref{eq:4.4}) represents Poisson's equation, which relates the spatial gradient of the electric field to the local charge density.
...
\noindent By substituting the conduction and displacement current components (\ref{eq:4.2}, \ref{eq:4.3}) and Poisson's equation (\ref{eq:4.4}) into the total current equation (\ref{eq:4.1}), the device current density is derived as (\ref{eq:4.5}). Even with a fixed applied voltage, the current remains a function of time because the internal electric field $E(x,t)$ evolves as the space charge redistributes. Integrating (\ref{eq:4.5}) with respect to $x$ from the cathode to the anode yields equation (\ref{eq:4.6}), which describes the current as a function of the electric field at the electrodes, defined as $E_a(t) = E(L,t)$ and $E_c(t) = E(0,t)$. \\

\noindent Note that the applied voltage was assumed to be non-varying with time, $\int_{0}^{L} \frac{\partial E(x,t)}{\partial t} dx = \frac{\partial }{\partial t} \int_{0}^{L}Edx = \frac{\partial V}{\partial t} = 0$, in a single dimension while \cite{lampert1970current}. This equation can further be rewritten with respect to the insulator total charge (\ref{eq:4.7}), with the relationship between the cathode and anode electric fields given as $E_a(t) = E_c(t) + \frac{q(t)}{\epsilon}$, where $Q(t)$ is the insulator total charge. \\
