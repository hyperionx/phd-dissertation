...
Before delving into brain-inspired spiking dynamics, it is necessary to establish the operational principles of conventional Artificial Neural Networks (ANNs) to understand the paradigms neuromorphic computing aims to replace. ANNs are structurally organized into discrete layers of artificial neurons, typically comprising an input layer, one or more hidden layers, and an output layer \cite{stone2019artificial}. In a standard feed-forward architecture, such as a Multilayer Perceptron (MLP), information propagates sequentially through fully connected synaptic layers. \\

\noindent The fundamental mathematical operation of these layers is matrix-vector multiplication. The output vector $\textbf{y}$ of a given layer is computed as $\textbf{y} = f(\textbf{xW} + \textbf{b})$, where $\textbf{x}$ is the input vector, $\textbf{W}$ is the synaptic weight matrix, $\textbf{b}$ is the bias vector, and $f$ is a non-linear activation function \cite{pal2010handwritten}. In traditional computing systems, performing these dense matrix-vector multiplications is highly constrained by the von Neumann bottleneck—the severe energy and latency costs associated with continuously shuttling large weight matrices between isolated memory and processing units \cite{zbontar2016stereo}. \\

\noindent The inclusion of a non-linear activation function is critical, as it expands the network's capacity to model highly complex, non-polynomial data representations \cite{pomerat2019neural}. A prominent activation function in modern deep learning is the Rectified Linear Unit (ReLU), defined as $f(z) = \max\{0, z\}$ \cite{zhang2018efficient}. ReLU effectively mitigates the vanishing gradient problem often associated with classical saturating functions like the logistic sigmoid or hyperbolic tangent, enabling the successful training of much deeper networks. \\

\noindent To optimize the parameters ($\textbf{W}$ and $\textbf{b}$), ANNs undergo a supervised training process aimed at minimizing a predefined loss function, such as cross-entropy or mean squared error \cite{kline2005revisiting}. The workhorse of this optimization is the backpropagation algorithm \cite{rumelhart1986learning}. Backpropagation solves the spatial credit assignment problem by calculating the exact gradient of the loss function with respect to every weight in the network, accomplished by recursively applying the chain rule backward from the network's output layer down to the input layer. \\

\noindent Guided by these gradients, an optimization algorithm—predominantly Stochastic Gradient Descent (SGD) or its variants—iteratively updates the weights in the direction of the negative gradient. By processing mini-batches of data over multiple epochs, SGD drives the network towards an optimal minimum in the loss landscape, allowing the ANN to generalize its learned features for accurate classification and inference on unseen data. While immensely powerful, the severe energy requirements and memory-transport bottlenecks of these conventional ANNs have driven researchers to seek vastly more efficient computing paradigms inspired directly by biology.

\subsection[Brain-like Analogy]{Brain-like Analogy}

The human brain processes information using vast networks of specialized cells called neurons. A biological neuron consists of three main functional parts: the dendrites (branching structures that act as receivers), the soma (the central cell body that integrates incoming signals), and the axon (a long cable that acts as a transmitter). Neurons connect to one another at tiny junctions called synapses, typically where the axon of a transmitting (presynaptic) neuron meets a dendrite of a receiving (postsynaptic) neuron. \\

\noindent Communication across these synapses relies on both chemical and electrical signals. When a presynaptic neuron fires an electrical spike (an action potential), it triggers the release of chemical messengers called neurotransmitters into the synapse. These chemicals bind to the postsynaptic dendrite, generating a temporary electrical voltage known as a postsynaptic potential. These localized voltage changes travel to the soma, perturbing its resting electrochemical equilibrium. If the accumulated voltage surpasses a specific threshold, the soma fires its own action potential down its axon. The efficiency of this signal transfer represents the synaptic weight or connection strength, which is dynamically adjusted by ionic flow during learning. Figure \ref{fig:6a} illustrates this biological network of chemical and electrical signaling.
...
\noindent In the realm of artificial intelligence, Spiking Neural Networks (SNNs) attempt to emulate this highly efficient, event-driven biological paradigm. Unlike traditional Artificial Neural Networks (ANNs), which communicate using continuous, high-precision numeric values, SNNs communicate using discrete voltage spikes. To physically implement this brain-like architecture in hardware, engineers use a hybrid of standard silicon transistors (CMOS devices) and memristors. 

\noindent Memristors are electronic components whose electrical resistance changes based on the history of the current flowing through them. This memory-like, accumulative behavior makes them ideal artificial synapses, as they can represent synaptic weight through adjustable electrical conductance \cite{chang2011short}. Figure \ref{fig:6b} depicts such a hybrid SNN architecture. In this configuration, standard CMOS circuits act as the artificial neurons (the soma and axon), while dense crossbar meshes of memristors act as the interconnecting synapses \cite{saighi2015plasticity}.
...
\noindent In this artificial circuit, the CMOS-based spiking neurons demonstrate functionality comparable to biological integrate-and-fire dynamics \cite{prezioso2016self}. The total input current reaching a receiving CMOS neuron is determined by Ohm's Law—calculated as the sum of the voltage drops across the connected memristors. If this accumulated current exceeds a programmed threshold, the CMOS neuron fires a spike. Crucially, this setup enables biologically plausible learning directly in the hardware. For instance, if a presynaptic CMOS neuron fires just before a postsynaptic CMOS neuron, the overlapping electrical pulses increase the memristor's conductance, effectively strengthening the artificial synapse. Reversing the spike order decreases the conductance, providing a mechanism for the network to continuously adapt its weights without external memory updates.

\subsection[Spiking Paradigm]{Spiking Paradigm}

Deep artificial neural networks (DNNs) represent a significant achievement in modern artificial intelligence, demonstrating remarkable efficacy not only in computer vision tasks like complex image recognition \cite{krizhevsky2012imagenet}, but also across diverse domains such as natural language processing, speech recognition, and autonomous decision-making. However, these conventional networks are engineered to operate exclusively on rate-based neurons. In rate-based models, neurons communicate using continuous, high-precision activation values (typically floating-point numbers) that represent the average firing rate of a neuron over a specific time window, abstracting away the precise temporal dynamics of individual impulses. This fundamental reliance on continuous values makes their translation to discrete, event-driven spiking neurons a challenging and highly active emerging research area.\\

\noindent The primary motivations for running such networks in spiking neurons are twofold. First, it enables the operation of large Convolutional Neural Network (CNN) models, which have recently succeeded in object recognition and other tasks, on spiking neuromorphic hardware. This facilitates the development of energy-efficient systems capable of real-time object recognition on robotic platforms, where current technology is too energy-intensive for mobile deployment. Second, it allows for incorporating additional brain-like components into machine learning models. \\

\noindent Neuroscience presents numerous challenges understanding regarding brain learning mechanisms, including neuronal nonlinear characteristics (especially firing thresholds) and the intricacies of spike-based communication (discreteness and variability). Although the spiking deep networks presented here are not intended as models of brain-like learning processes, they address challenges also faced by the brain.This section offers insights that motivate the development of more biologically plausible learning mechanisms.\\
\noindent Understanding biological learning mechanisms remains a profound challenge in neuroscience, largely due to neuronal nonlinearities, such as dynamic firing thresholds, and the complexities of discrete, variable spike-based communication. While the artificial spiking networks discussed here are not perfect biological replicas, they confront identical computational challenges—namely, how to efficiently process sparse, noisy, and asynchronous events. Overcoming these hurdles provides critical insights that motivate the development of more biologically plausible neuromorphic systems.\\

\noindent Spiking deep networks employ information transmission between neurons as discrete spikes. A key distinction of spiking networks is their inclusion of an additional temporal dimension, which is typically absent in rate-based DNNs. In these rate-based networks, continuous activation values are used to abstractly represent the average firing rate of spikes, rather than the individual spikes themselves. A spiking neuron evolves over time, sometimes emitting spikes and sometimes not. This process can only be analyzed over time; examining it instantaneously provides limited insight. In contrast, in rate-based networks, once an input is presented, the activities of subsequent neurons can be instantaneously determined, as they represent a static firing rate that does not change dynamically over time, unless the rate changes.\\

\noindent A second notable distinction in artificial spiking networks is that spikes are treated as discrete and identical events. Although biological action potentials can exhibit subtle variations in shape and amplitude, standard SNN models abstract them into uniform pulses. Consequently, the primary information conveyed by a spike is simply its occurrence time. This implies two primary categories of codes a neuron can use: rate codes and timing codes. Timing codes focus on individual spike times, while rate codes focus on the number of spikes within a specific time window, or potentially their relative timing. When examining the number of spikes within a specified time interval, the resulting rate is inherently discrete. For \textit{n} spikes within a \textit{t}-second window, the firing rate is \textit{n/t}, where \textit{n} is an integer. For a fixed \textit{t}, the firing rate can only assume a discrete set of values, specifically integer values of \textit{n}. \\

\noindent A third distinction concerns the inherent variability of spiking neuron output, which differs significantly from traditional rate-based counterparts (such as conventional Artificial Neural Networks). It is important to clarify that a rate-based network is not a type of Spiking Neural Network (SNN); rather, it abstracts the discrete nature of spikes by using continuous activation values to represent the average firing rate over time. With a constant input, a rate neuron provides a constant continuous output. While some rate models can exhibit internal dynamics (e.g., adapting rate-based Leaky Integrate-and-Fire (LIF) models), their output remains considerably less variable than that of actual spiking neurons. In contrast, spiking neurons output a discrete spike train, which, when filtered by a synapse, results in an oscillating signal whose variability depends on the underlying spike rate. Consequently, even with constant input, a spiking network will exhibit dynamic variability in both neuron inputs and overall network outputs.\\

\noindent DNNs are typically formulated as rate-based models, where nonlinear activity represents a neuron's firing rate. A Rectified Linear Unit (ReLU) activation function, for instance, can be conceptualized as a neuron silent when its input is less than zero, and whose firing rate increases linearly as current rises above zero. Furthermore, cost functions for rate-based DNNs are based on output neuron firing rates. For classification, the chosen class is the output unit with the highest activity. One approach is to treat spiking networks similarly, training the network so the unit corresponding to the target class has the highest activity. This activity does not necessarily correspond to a neuron's firing rate; it is more likely the filtered, weighted sum over the spiking activities of the final neuron layer. Alternatively, the network can be trained so the first neuron to spike represents the chosen class.\\
...
\noindent Synapses form the connections between neurons, modulating the membrane potential of the postsynaptic neuron upon receiving a spike from a presynaptic neuron. These connections are broadly classified as either excitatory or inhibitory \cite{kandel2000principles}. Excitatory synapses cause a depolarization of the postsynaptic membrane, increasing the likelihood of firing, whereas inhibitory synapses cause hyperpolarization, decreasing this likelihood. The magnitude of this potential change is determined by the synaptic weight, or strength, of the connection. In conventional Artificial Neural Networks (ANNs), these weights are typically fixed after an offline training phase. In contrast, Spiking Neural Networks (SNNs) can leverage synaptic plasticity, allowing weights to evolve over time in response to network activity, a key feature for online and continuous learning \cite{tavanaei2019deep}.\\
...
\noindent Figure \ref{fig:6c} illustrates the temporal dynamics of a spiking neuron, where inputs, weights, and outputs are all functions of time. The neuron receives a spike train, where each incoming spike is modulated by a synaptic weight that either amplifies or dampens its effect. Each weighted spike contributes to the neuron's membrane potential, which accumulates charge over time. When the potential reaches a predefined threshold, the neuron fires an output spike. Immediately following the spike, the membrane potential is reset, and the neuron enters a brief refractory period during which it cannot fire again. After this period, the integration process resumes. This behavior can be defined mathematically as follows:
\begin{equation}
    V_m(t) = V_m(t-1) + \sum_{i} w_{i,j} \times x_{i,j}(t-1) - \lambda
    \label{eq:6.1}
\end{equation}

\noindent Equation \ref{eq:6.1} describes the discrete-time evolution of the membrane potential, $V_m(t)$, where the potential at the next time step is the sum of the previous potential, $V_m(t-1)$, and the weighted inputs from presynaptic neurons, $w_{i,j} \times x_{i,j}(t-1)$, minus a constant leakage term, $\lambda$. If the membrane potential exceeds a predefined firing threshold, the neuron emits a spike and its potential is reset to a resting value, $V_{rest}$. Following a spike, the neuron enters a refractory period during which it cannot fire again, though its membrane potential can still accumulate incoming charge.
\begin{equation}
    x(t) = 
    \begin{cases} 
...
\noindent Equation \ref{eq:6.2} defines the firing condition, where a neuron fires only if its membrane potential exceeds the threshold voltage, $V_{th}$, and the elapsed time since its last spike, $t_{spike}$, is greater than the refractory period, $T_R$. While the Leaky Integrate-and-Fire (LIF) model is popular for its simplicity \cite{lapique1907researches}, more biorealistic models such as the Hodgkin-Huxley \cite{hodgkin1952quantitative} and Izhikevich models \cite{izhikevich2003simple} offer greater fidelity to the behaviour of real neurons. However, these models rely on complex differential equations that are challenging to implement efficiently in hardware, which motivates the use of simplified neuron models for practical neuromorphic systems.\\



\noindent A key challenge in applying SNNs is the encoding of continuous, real-world data into the discrete, binary spike trains that these networks process. A prevalent method is rate-coding, where an analog value is translated into a corresponding neuronal firing rate, typically averaged over a specific time window \cite{gerstner1997neural}. This can be expressed mathematically as follows:
\begin{equation}
    X(t) = 
    \begin{cases} 
    1 & \text{if } x < u \\ 
...
\noindent where $u$ is a random variable drawn from a uniform distribution $\mathcal{U}(0,1)$ at each time step $t$. However, rate-coding methods have significant drawbacks. By averaging information over a time window, they discard the precise timing of individual spikes, which is a critical component of neural information processing in many biological systems \cite{chase2006spike}. This random, probabilistic generation of spikes can lead to sparse or dense spike trains that lack the rich temporal structure found in biological signals. In contrast, temporal coding schemes leverage the precise timing of spikes to encode information. Prominent examples include Time-to-First-Spike (TTFS), where information is encoded in the latency of the first spike, and Inter-Spike Interval (ISI) coding, where the intervals between consecutive spikes carry information \cite{gerstner2014neuronal}.
